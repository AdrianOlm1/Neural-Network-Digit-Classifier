...
def preprocess(cell):
    """Preprocess Sudoku cell for CNN."""
    cell = cv2.resize(cell, (28, 28))  # Resize to 28x28
    cell = cv2.GaussianBlur(cell, (3, 3), 0)  # Reduce noise

    # Convert to black background with white digits
    _, cell = cv2.threshold(cell, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    
    #kernel = np.ones((3,3), np.uint8)  # Adjust kernel size to avoid excessive thickening
   # cell = cv2.dilate(cell, kernel, iterations=1)  # Dilate to restore eroded digits

    # Convert to PyTorch tensor
    cell = Image.fromarray(cell)
    cell = transforms.ToTensor()(cell)
    
    # Normalize (MNIST uses mean=0.1307, std=0.3081)
    cell = transforms.Normalize((0.1307,), (0.3081,))(cell)  # MNIST normalization
